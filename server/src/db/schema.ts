import { pgTable, uniqueIndex, uuid, text, timestamp, foreignKey, integer, pgEnum } from "drizzle-orm/pg-core"
  import { sql } from "drizzle-orm"

export const userStatus = pgEnum("UserStatus", ['ACTIVE', 'PENDING', 'DISABLED'])
...
(table) => {
	return {
		...
		shortIdKey: uniqueIndex("user_shortId_key").using("btree", table.shortId.asc().nullsLast()),
	}
});
...
export const goal = pgTable("goal", {
	...
	targetDate: timestamp("targetDate", { precision: 3, mode: 'string' }).notNull(),
	createdAt: timestamp("createdAt", { precision: 3, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	updatedAt: timestamp("updatedAt", { precision: 3, mode: 'string' }).notNull(),
},
(table) => {
	return {
		shortIdKey: uniqueIndex("goal_shortId_key").using("btree", table.shortId.asc().nullsLast()),
...
export const entry = pgTable("entry", {
	...
	date: timestamp("date", { precision: 3, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	createdAt: timestamp("createdAt", { precision: 3, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	updatedAt: timestamp("updatedAt", { precision: 3, mode: 'string' }).notNull(),
},
(table) => {
	return {
		shortIdKey: uniqueIndex("entry_shortId_key").using("btree", table.shortId.asc().nullsLast()),
